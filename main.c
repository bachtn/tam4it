...
{
  const char* output = "./Result/output.bmp";
...
void extract_data_from_image(SDL_Surface *img)
{
  const char* output = "./Result/output.bmp";
  char* output_detected = "./Result/detection.bmp";
...
int interface(int argc, char **argv, SDL_Surface *img)
{
  ...
  pImage = NULL;
	...
	g_signal_connect(G_OBJECT(pButton[2]), "clicked", G_CALLBACK(greyscale), img);
	g_signal_connect(G_OBJECT(pButton[3]), "clicked", G_CALLBACK(binary), img); 
	//g_signal_connect(G_OBJECT(pButton[4]), "clicked", G_CALLBACK(flou), img); 
	//g_signal_connect(G_OBJECT(pButton[5]), "clicked", G_CALLBACK(rotation), img);  
	g_signal_connect(G_OBJECT(pButton[1]), "clicked", G_CALLBACK(extract_data_from_image), img);  
